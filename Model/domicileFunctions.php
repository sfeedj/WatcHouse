...
function uniteMesure($id,$bdd) {
  $req=$bdd->prepare("SELECT Type FROM capteurs WHERE UUID=?");
  $req->execute(array($id));
  $res=$req->fetch();
  
  if ($res['Type']=='WatcHouse Hygromètre') {
    return ' %';
  }
  elseif ($res['Type']=='WatcHouse Luxmètre') {
    return ' Lux';
  }
  elseif ($res['Type']=='WatcHouse Thermomètre') {
    return ' °C';
  }
